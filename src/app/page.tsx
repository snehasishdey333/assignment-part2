...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const types = Array.from(new Set(hospitalityData.map((exp) => exp.propertyType)))
    setPropertyTypes(types)

    const allYears = Array.from(new Set(hospitalityData.map((exp) => exp.duration)))
    setYears(allYears)
  }, [])
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold text-green-800 mb-2">Hospitality Experience Explorer</h1>
        <p className="text-gray-600 mb-4">
          Browse through professional experiences and projects in the hospitality industry 
        </p>
        <p className="text-gray-600 mb-8 font-bold">Part 2</p>

        <FilterBar propertyTypes={propertyTypes} years={years} onFilter={handleFilter} />
