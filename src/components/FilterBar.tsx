      ...
      <div className="bg-white p-4 rounded-lg shadow-sm mb-6">
        <div className="flex flex-col md:flex-row md:items-center gap-4">
          <div className="flex-1">
            <p className="block text-sm font-bold text-gray-700 mb-1">Property Type</p>
            <div className="flex flex-wrap gap-2">
              {propertyTypes.map((type) => (
                <button
                  key={type}
                  onClick={() => handlePropertyTypeChange(selectedPropertyType === type ? null : type)}
            ...
            <div className="flex flex-wrap gap-2">
              {years.map((year) => (
                <button
                  ...
                  className={`px-3 py-1 text-sm rounded-full ${
                    selectedYear === year ? "bg-green-600 text-white" : "bg-gray-100 text-gray-800 hover:bg-gray-200"
                  }`}
                >
                  {year}
                </button>
